...
\begin{algorithmic}
\STATE{{\bf If} $S$ does not meet the stopping criterion:}
\FOREACH{attribute $A$ in $List_A$ }
\STATE{get the split of values of $A$ that yields the smallest impurity as measured by $I$}
\ENDFOR
\STATE{split $S$ into $(L, R)$ using the attribute $A$ whose best split has the smallest impurity}
\STATE{CreateTree($L$)}
\STATE{CreateTree($R$)}
\end{algorithmic}
\end{algorithm}
